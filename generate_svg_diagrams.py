...
def create_hero_svg(output_svg):
  ...
  <text x="52" y="396" class="txt-bold" fill="#93C5FD">NES Internal RAM Execution Landing ($03D0) — 60-Byte Visual Payload Routine</text>

  <text x="52" y="430" class="txt-bold" fill="#F59E0B">1. Vblank Wait</text>
  <text x="220" y="430" class="txt-code-bold" fill="#38BDF8">BIT $2002; BPL -3</text>
  <text x="510" y="430" class="subtitle">Synchronizes CPU writes safely with PPU vertical blanking</text>

  <text x="52" y="460" class="txt-bold" fill="#F59E0B">2. Palette Overwrite</text>
  <text x="220" y="460" class="txt-code-bold" fill="#38BDF8">STA $2006 ($3F00); STA $2007</text>
  <text x="510" y="460" class="subtitle">Overwrites PPU palette memory ($3F00-$3F1F) with dynamic HSV colors</text>

  <text x="52" y="490" class="txt-bold" fill="#F59E0B">3. PPU Reset</text>
  <text x="220" y="490" class="txt-code-bold" fill="#38BDF8">STA $2006 ($2000)</text>
  <text x="510" y="490" class="subtitle">Restores PPU address registers back to nametable for normal rendering</text>

  <text x="52" y="520" class="txt-bold" fill="#F59E0B">4. State Mutators</text>
  <text x="220" y="520" class="txt-code-bold" fill="#38BDF8">STA $079F=#$FF; STA $0756=#$02</text>
  <text x="510" y="520" class="subtitle">Sets Star Invincibility timer to 255 and promotes Mario to Fire Mario</text>

  <text x="52" y="550" class="txt-bold" fill="#F59E0B">5. Marker Assertion</text>
  <text x="220" y="550" class="txt-code-bold" fill="#38BDF8">STA $07FF=#$85; RTS</text>
  <text x="510" y="550" class="subtitle">Asserts diagnostic marker byte ($85) and cleanly returns to game engine</text>
</svg>
"""
